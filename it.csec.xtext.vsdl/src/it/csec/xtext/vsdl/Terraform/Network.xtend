...
class Network extends ScenElem {
	...
	'''
	resource "openstack_networking_network_v2" "''' + this.name + '''" {
	  name = "''' + this.name + '''"
	  admin_state_up = "true"
	}
	
	resource "openstack_networking_subnet_v2" "subnet_''' + this.name +'''" {
	  name = "subnet_''' + this.name + '''"
	  network_id = "${openstack_networking_network_v2. ''' + this.name + '''.id}"
	  cidr = "''' + this.address + '''/''' + this.netmask + '''"
	  ip_version = 4
